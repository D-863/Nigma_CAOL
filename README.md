...
This enables Modules to implement various instructions that the Compiler may not enable like SIMD, sin, sqrt, etc instructions; Or even Higher-Level operations from various Subjects! Like Calculus, Linear Algebra, Geometric Algebra and so on.. (The list really is infinite.)

Without having to worry about the fundamental steps of each operation, if the Architecture has a hardware alternative or how each Compiler enables the injection of instruction.
